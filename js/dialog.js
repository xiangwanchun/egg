...
// 途牛无线 M站
//
// Copyright (c) 2013 Tuniu Inc.
/////////////////////////////////

/**
 * 功能說明：dialog v0.1, 基于zepto
 * @author: 周海 zhouhai (zhouhai@tuniu.com)
 * @update: 2014-01-22 11:11
 */
	...
	function dialog(options) {
		...
		c.dialogCss = '.dialog-box{opacity:0;position: fixed;left: ' + c.ow * 0.2 + 'px;width: 60%;border:2px solid #505050;z-index: 999;font-size:' + c.fontSize + ';color:' + c.color + ';background-color:#363636;color:#fff;text-shadow:0 0 3px #000;' + (noShadow() ? '' : '-webkit-box-shadow:0 2px 3px rgba(0,0,0,0.5);border-radius:16px;') + '}' +
			...
			'.dialog-body{padding:20px;font-size:' + opts.messageSize + ';}' +
			'.dialog-footer{height: 80px; border-top: 1px solid #4b4b4b;font-size:' + opts.messageSize + ';}' +
			'.dialog-ok{width: 50%;text-align: center;float: left;line-height: 80px;}' +
			'.dialog-cancel{width: 49%;border-left: 1px solid #4b4b4b;text-align: center;float: right;line-height: 80px;}' +
		...
		publicFuncs.show = function() {
			if (opts.type == 'info') {
				...
				showtimeout = setTimeout(publicFuncs.hide, 2000);
